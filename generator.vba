Sub GeneratePDFsFromCSV()
    ...
    pdfPath = ThisWorkbook.Path & "\generated-" & Format(Now, "yyyy-MM-dd_HH-mm-ss") & "\"
    ...
    For i = 2 To lastRow
        ' Create a new PDF for each row
        Set pdf = CreateObject("Excel.Application")
        pdf.Visible = False
        ...
        ' Close the PDF application without saving changes
        pdf.Quit
    Next i
